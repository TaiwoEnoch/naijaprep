...
export async function GET(req: NextRequest, { params }: { params: { questionId: string } }) {
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: "user",
        content: `You are a Nigerian exam tutor for SS3 students preparing for ${question.exam_type}.
...
Correct answer: ${question.correct_option}`,
    ...
    if (response.content && response.content[0] && response.content[0].type === "text") {
      ...
      throw new Error("Invalid content format in Anthropic API response")
    }
